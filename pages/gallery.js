...
const CATEGORIES = [
  ...
  { value: 'tropicalFoliage', label: 'Tropical Foliage' },
  { value: 'exoticBlooms', label: 'Exotic Blooms' },
...
export async function getStaticProps() {
  ...
  try {
    if (sanityClient) {
      const query = `*[_type == "collectorGallery" && (slug.current == "gallery" || slug.current == "orchids-gallery")][0]{
        title,
        intro,
        seo,
        items[]{
...
export default function Gallery({ initialImages = [], pageContent = null }) {
  ...
  return (
    <div style={{ background: '#00301E', minHeight: '100vh', padding: '3rem 1.5rem', color: '#E9DCBE' }}>
      <Head>
        <title>{pageContent?.seo?.title || 'Botanical Collector Gallery | The Botanical Bazaar St. Petersburg FL'}</title>
        <meta name="description" content={pageContent?.seo?.description || 'High-resolution visual gallery of rare tropical specimens, collector orchids, variegated aroids, exotic blooms, and fruit trees grown at The Botanical Bazaar.'} />
        <link rel="canonical" href="https://thebotanicalbazaar.com/gallery" />
        <meta property="og:title" content="Botanical Collector Gallery | The Botanical Bazaar" />
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ fontSize: '1.2rem', fontStyle: 'italic', color: '#F5E7C4', maxWidth: '700px', margin: '0 auto 1.5rem' }}>
            {pageContent?.intro || 'High-resolution photographic highlights of rare orchids, mature tropical specimens, exotic blooms, and unique varieties nurtured at our St. Petersburg nursery.'}
          </p>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8rem', justifyContent: 'center', marginBottom: '2.5rem' }}>
          {CATEGORIES.map((cat) => (
            <button
              ...
              style={{
                backgroundColor: activeCategory === cat.value ? '#D4B06A' : '#1C3D2E',
                color: activeCategory === cat.value ? '#00301E' : '#D4B06A',
                ...
                boxShadow: activeCategory === cat.value ? '0 4px 12px rgba(212, 176, 106, 0.3)' : 'none'
              }}
            >
              {cat.label}
            </button>
