...
const defaultCareSheets = [
  {
    ...
    humidityNeeds: '50% - 70% Humidity',
    soilNeeds: 'Coarse Orchid Bark Mix',
  ...
  },
  {
    ...
    humidityNeeds: '40% - 60% Humidity',
    soilNeeds: 'Rich Organic Well-Draining Soil',
  ...
  },
  {
    ...
    humidityNeeds: 'Tolerates High Subtropical Humidity',
    soilNeeds: 'Compost-Rich Sandy Loam',
    toxicity: 'Foliage mildly toxic if ingested by pets',
...
export default function CareSheets({ careSheets }) {
  ...
  return (
    <div style={{ padding: '3rem 1.5rem', maxWidth: '1050px', margin: '0 auto', color: '#E9DCBE' }}>
      <Head>
        <title>Plant Care Sheets | The Botanical Bazaar</title>
        <meta name="description" content="Explore tropical plant care sheets, cultivation instructions, and botanical guides curated for St. Petersburg growers by The Botanical Bazaar." />
        <link rel="canonical" href="https://thebotanicalbazaar.com/care-sheets" />
        <meta property="og:title" content="Plant Care Sheets | The Botanical Bazaar" />
        <meta property="og:description" content="Explore tropical plant care sheets, cultivation instructions, and botanical guides curated for St. Petersburg growers." />
      ...
      <p style={{ textAlign: 'center', fontSize: '1.15rem', maxWidth: '750px', margin: '0 auto 2.5rem auto', lineHeight: '1.6', fontStyle: 'italic' }}>
        Comprehensive care guidelines, environmental specs, and seasonal advice for our acclimated tropical plant collection.
      </p>
          ...
          {sheets.map((sheet, i) => (
            <div key={i} style={{ background: '#1C3D2E', borderRadius: '12px', border: '1px solid #D4B06A', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'space-between' }}>
                <div>
                  <h2 style={{ color: '#D4B06A', margin: '0 0 0.2rem 0', fontFamily: 'Cinzel, serif', fontSize: '1.35rem' }}>
                    {sheet.commonName}
                  </h2>
                  ...
                  <div style={{ background: '#00301E', padding: '0.8rem 1rem', borderRadius: '8px', fontSize: '0.88rem', marginBottom: '1rem', border: '1px solid #2d5a44', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                    {sheet.lightNeeds && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>Light:</strong> {sheet.lightNeeds}</p>}
                    {sheet.wateringNeeds && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>Watering:</strong> {sheet.wateringNeeds}</p>}
                    {sheet.humidityNeeds && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>Humidity:</strong> {sheet.humidityNeeds}</p>}
                    {sheet.soilNeeds && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>Soil:</strong> {sheet.soilNeeds}</p>}
                    {sheet.toxicity && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>Pet Safety:</strong> {sheet.toxicity}</p>}
                    {sheet.zoneCompatibility && <p style={{ margin: 0 }}><strong style={{ color: '#D4B06A' }}>USDA Zones:</strong> {sheet.zoneCompatibility}</p>}
                  </div>
