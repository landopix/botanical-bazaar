function getSanitizedShopDomain(str) {
  ...
  return domain || 'shop.thebotanicalbazaar.com';
}
...
export function formatShopifyProduct(node) {
  ...
  const resolvedHardinessZones = (node.taxonomyHardinessZoneMetafield?.references?.nodes || [])
    .map(zone => zone.label?.value)
    .filter(label => typeof label === 'string' && /^(?:[1-9]|1[0-3])[ab]$/i.test(label.trim()));
  // A list.metaobject_reference value contains GIDs, never usable zone labels.
  const taxonomyHardinessZoneValue = resolvedHardinessZones.length
    ? JSON.stringify(resolvedHardinessZones)
    : node.taxonomyHardinessZoneMetafield?.type === 'list.metaobject_reference'
      ? null : node.taxonomyHardinessZoneMetafield?.value || null;
  ...
  if (taxonomyHardinessZoneValue) {
    if (typeof taxonomyHardinessZoneValue === 'string') {
      try {
        const parsed = JSON.parse(taxonomyHardinessZoneValue);
        if (Array.isArray(parsed)) {
          parsed.forEach(z => zones.add(String(z).trim()));
        } else {
          zones.add(String(parsed).trim());
        }
      } catch (e) {
        zones.add(taxonomyHardinessZoneValue.trim());
      }
    }
  }
  ...
  return {
    ...
    custom: {
      ...
      taxonomy_hardiness_zone: taxonomyHardinessZoneValue,
...
const PRODUCT_METAFIELDS_FRAGMENT = `
  ...
  taxonomyHardinessZoneMetafield: metafield(namespace: "shopify", key: "hardiness-zone") {
    value
    type
    references(first: 30) {
      nodes {
  ...
  try {
    ...
    const articles = data?.blog?.articles?.edges?.map(edge => {
      ...
      return {
        ...
        onlineStoreUrl: node.onlineStoreUrl ? node.onlineStoreUrl.replace(/^https?:\/\/[^/]+\.(?:myshopify\.com|thebotanicalbazaar\.com)/, "https://thebotanicalbazaar.com").replace(/\/blogs\/[^/]+\//, "/almanac/").replace(/\/products\//, "/product/") : (node.handle ? `https://thebotanicalbazaar.com/almanac/${node.handle}` : null),
...
/**
 * Fetches one published Shopify article from the Almanac blog by handle.
 */
export async function getAlmanacArticle(articleHandle, blogHandle = 'the-almanac') {
  const query = `
  ...
  return {
    ...
    author: article.authorV2?.name || 'The Botanical Bazaar',
...
/**
 * Fetches menu navigation structure from Shopify Storefront API by handle.
 * Filters out collection links that have 0 active/published products for SEO thin-content protection.
 */
export async function getShopifyMegaMenu(menuHandle = 'master-mega-menu') {
  const query = `
    query getShopifyMenu($handle: String!) {
      menu(handle: $handle) {
        id
        title
        handle
  ...
  try {
    const data = await shopifyFetch({ query, variables: { handle: menuHandle } });
    const rawItems = data?.menu?.items || [];

    const processItem = (item) => {
      const isCollection = item.type === 'COLLECTION' || Boolean(item.resource?.handle);
      const productCount = item.resource?.products?.edges?.length || 0;

      // Conditional rendering requirement: hide collection link if active product count is 0
      if (isCollection && productCount === 0) {
        return null;
      }
      ...
      if (relativeUrl.startsWith('https://') || relativeUrl.startsWith('http://')) {
        try {
          const parsed = new URL(relativeUrl);
      ...
      return {
        ...
        title: item.title,
        url: relativeUrl,
        handle: item.resource?.handle || null,
        items: childItems,
        productCount: isCollection ? productCount : null,
      };
    };

    return rawItems.map(processItem).filter(Boolean);
  } catch (err) {
    console.error(`Error fetching Shopify menu "${menuHandle}":`, err);
    return [];
  }
}
