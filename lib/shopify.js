...
export function formatShopifyProduct(node) {
  ...
  const minTempInGround = minTempGroundMetafield
    ? `${minTempGroundMetafield}°F`
    : null;

  const minTempInPot = minTempPotMetafield
    ? `${minTempPotMetafield}°F`
    : null;
  ...
  const petSafe = petSafeMetafield === 'true' || petSafeMetafield === true;
...
/**
 * GraphQL fragment for botanical namespace metafields
 */
const BOTANICAL_METAFIELDS_FRAGMENT = `
