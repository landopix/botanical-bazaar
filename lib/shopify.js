  ...
  try {
    ...
    const articles = data?.blog?.articles?.edges?.map(edge => {
      ...
      return {
        ...
        onlineStoreUrl: node.onlineStoreUrl ? node.onlineStoreUrl.replace(/^https?:\/\/[^/]+\.(?:myshopify\.com|thebotanicalbazaar\.com)/, "https://thebotanicalbazaar.com").replace(/\/products\//, "/product/") : null,
